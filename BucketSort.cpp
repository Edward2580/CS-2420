...
void step1() {
    ...
    if (numBuckets == 1) {
        ...
        unsigned int rangePerBucket = (largest / numBuckets) + 1;

        for (unsigned int i = 0; i < arrSize; i++) {
            buckets[arr[i] / rangePerBucket].push_back(arr[i]);
        }
    }
...
void step2() {
   ...
   // sortOneVector(buckets[currentBucket]);
    ...
    while (true) {
        myMutex.lock();
        threadNumber = currentBucket;
        currentBucket++;
        myMutex.unlock();

        if (threadNumber >= numBuckets) {
            return;
        }
        else {
            sortOneVector(buckets[threadNumber]);
        }
...
void step3() {
    ...
    // Index of your next open spot in the output array
      ...
      //tracker for the array
    unsigned int tracker{ 0 };

    //looping through the buckets
    for (unsigned int i = 0; i < numBuckets; i++) {

        //getting the size of the buckets
        unsigned int item = buckets[i].size();

        //looping through the specific bucket
        for (unsigned int j = 0; j < item; j++) {
            arr[tracker] = buckets[i][j];
            tracker++;
        }
    }
}
...
void bucketSort() {
   ...
   /* for (unsigned int currentBucket = 0; currentBucket < numBuckets; currentBucket++) {
        step2(currentBucket);
    }*/
